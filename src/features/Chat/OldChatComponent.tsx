...
const ChatComponent = () => {
  ...
  const canDeleteMess =  user?.role == `MODERATOR`
  ...
  const PostComponent = ({ post }:{ post: ChatPost }) => {
    return (
      <Box sx={{ padding:`10px`, borderBottom:`1px solid`, borderColor:`primary.main`, display:`flex`, justifyContent:`space-between` }}>
        ...
        <Button variant='contained' color="error" onClick={() => handleClick( post.id )} disabled={!canDeleteMess}> Usuń </Button>
      </Box>
    )
  }
...
type x = [...[unknown, unknown], ...[unknown, unknown, unknown]]['length']
